...
var njs = {
    ...
    forEachArrayCallback: function forEachArrayCallback(
        arr,
        readyCallback,
        func,
    ) {
    ...
    formatValue: function (value, decimals, _format) {
        ...
        var ret = isNaN(value)
            ...
            : value
                .toFixed(decimals || 0)
                .replace(_format[0], _format[1])
                .replace(/\B(?=(\d{3})+(?!\d))/g, _format[0]);
        return ret;
    },
};
...
function Devices(_adapter, _callback) {
    ...
    this.update = function (list, callback) {
        ...
        if (!list || list.length == 0) {
            return safeCallback(callback, -1);
        }
    ...
    this.CDevice = function CDevice(_name, showName, list) {
                ...
                if (!that.has(id)) {
                    if (typeof showNameOrObject == "object") {
                        ...
                        var obj = {
                            type: "channel",
                            _id: id,
                            common: {name: showNameOrObject || name},
                        };
                    }
                    return push(obj);
                }
        ...
        function split(id, valOrObj, showName) {
            var ar = (
                id && id[0] == "." ? id.substr(1) : dcs(deviceName, channelName, id)
            ).split(".");
        ...
        this.setex = function (id, newObj, showName) {
            if (
                adapter &&
                id.substr(0, adapter.namespace.length) == adapter.namespace
            ) {
                id = id.substr(adapter.namespace.length + 1);
            }
            return this.set(id, newObj, showName);
...
function savePrevVersion() {
    if (!hasProp(adapter, "ioPack.common.version")) {
        return;
    }
    ...
    function set() {
        adapter.setForeignState(vid, {
            val: adapter.ioPack.common.version,
            ack: true,
            from: id,
        });
    }
...
function switchToDebug(force) {
    ...
    if (!force && !process.env.SOEF_DEBUG) {
        return;
    }
...
exports.Adapter = function (_args) {
    ...
    if (!options.unload) {
        options.unload = function (callback) {
            try {
                fns.onUnload ? onUnload(callback) : callback && callback();
            } catch (e) {
                callback && callback();
            }
        };
...
function changeAdapterConfig(_adapter, changeCallback, doneCallback) {
    _adapter.getForeignObject(
        `system.adapter.${_adapter.namespace}`,
        function (err, obj) {
            ...
            if (!err && obj && changeCallback(obj.native) !== false) {
                _adapter.setForeignObject(obj._id, obj, {}, function (err, s_obj) {
                    _adapter.log.debug("soef.changeAdapterConfig: changed");
...
exports.TimeDiff = function () {
    if (!(this instanceof exports.TimeDiff)) {
        return new exports.TimeDiff();
    }
...
function forEachObjectChild(id, options, readyCallback, callback) {
    ...
    function doDevices() {
        forEachInSystemObjectView(
            "device",
            id,
            doChannels,
            function (o, next, type) {
                callback(
                    o,
                    function () {
