...
8. [Yamaha Adapter Specifics](#yamaha-adapter-specifics)
...
### Yamaha-Specific Testing Considerations
- Mock external dependencies (HTTP requests, SSDP)
- Test with various receiver models when possible
- Verify network communication patterns
- Test state updates and synchronization
- Validate configuration options

### Yamaha Adapter Patterns

#### Configuration Validation
Always validate Yamaha receiver configuration:
```javascript
if (!this.config.ip) {
...
- `request`: HTTP request handling (note: deprecated, consider migrating to fetch API)
