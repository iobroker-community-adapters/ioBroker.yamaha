...
export interface YncaAvailSnapshot {
  /**
   * The discovery schema the snapshot was taken under (see `DISCOVERY_SCHEMA`) — a snapshot of
   * another schema is dropped on load like one of another model, so a changed discovery logic
   * re-probes the device.
   */
  schema: number;
...
export interface YncaSubunitCache {
  ...
  /** Store a fresh probe result, stamped with the current discovery schema (also persists it). */
  set(snapshot: Omit<YncaAvailSnapshot, "schema">): void;
 ...
 * object's native part is untrusted storage) of the CURRENT discovery schema.
 *
 * @param value the raw persisted value
 * @returns true when it carries the snapshot shape and the current schema
 */
...
export function createSubunitCache(
  ...
  return {
    get: () => current,
    set: snapshot => {
      current = { schema: DISCOVERY_SCHEMA, ...snapshot };
      persist(current);
    },
    clear: () => {
      current = undefined;
