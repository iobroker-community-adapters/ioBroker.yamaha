// The fleet's master test for the error-text helper (Entwicklung/CLAUDE_CODING.md, taken verbatim; the
// adapter keeps its name `errorMessage`, bound to `errText` here). Audit 2026-09-24, E1: the old form lost
// `cause` and `code` — "fetch failed" instead of the socket reason.
import http from "node:http";
import net from "node:net";
import { errorMessage as errText } from "./util";
