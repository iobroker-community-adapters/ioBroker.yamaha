...
const AMP_FUNCS = [
  ...
  },
  {
    ...
    wireEncode: value =>
      Number(value) === 16.5 ? "16.5" : (0, import_value_coerce.formatWireNumber)(Number(value), 1, 5),
...
function funcToEntry(entries) {
  return new Map(
    entries
      ...
      .flatMap(entry => readFuncsOf(entry).map(func => [`${entry.subunit}:${func}`, entry])),
  );
}
