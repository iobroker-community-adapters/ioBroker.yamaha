...
function buildYncaCatalog() {
  ...
  for (const key of TRIGGER_INPUT_KEYS) {
    entries.push({
      ...
      nameArgs: [(_a = INPUT_NAME_LABELS[key]) != null ? _a : key.toUpperCase()],
      spec: { kind: "enum", states: selfMap(["Lo", "Hi"]) },
      write: true,
      role: "state",
      subunit: "SYS",
  ...
  for (const key of INPUT_NAME_KEYS) {
    ...
    entries.push({
      ...
      nameArgs: [(_b = INPUT_NAME_LABELS[key]) != null ? _b : upper],
      spec: { kind: "text" },
      write: false,
      role: "text",
      subunit: "SYS",
  ...
  for (const source of ["ipod", "ipodUsb"]) {
    entries.push({
